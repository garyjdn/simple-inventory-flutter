...
class _TmpRequestItemMainState extends State<TmpRequestItemMain> {
  ...
  search(List<RequestItem> source, String keyword) {
    List<RequestItem> requests = [...source.where((request) {
      return request.requestUser.name.toLowerCase().contains(keyword.toLowerCase()) 
          || request.station.name.toLowerCase().contains(keyword.toLowerCase());
    }).toList()];
    if(mounted)
    setState(() => filteredRequest = requests);
  }
                      ...
                      TextFormField(
                        ...
                        onChanged: (value) {
                          if(value.isNotEmpty) {
                            ...
                            _debounce = Timer(const Duration(milliseconds: 500), () => search(state.requestItems, value));
                          } else {
                            setState(() => filteredRequest = state.requestItems);
                          }
                        },
                      ),
