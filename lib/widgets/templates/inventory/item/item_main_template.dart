          ...
          onPressed: () async {
            final fetch = await Navigator.of(context).pushNamed(
                ...
                arguments: ItemFormScreenArguments(
                    title: 'Add Item', action: 'create'));

            if (fetch != null && fetch) _itemBloc.add(LoadItemStarted());
          },
        ),
        body:
            BlocConsumer<ItemBloc, ItemState>(listener: (context, state) async {
          ...
          if (state is ItemLoadStarted || state is ItemLoadSuccess) {
            return true;
          }
          return false;
                              ...
                              IconButton(
                                onPressed: () async {
                                  final fetch = await Navigator.of(context)
                                      .pushNamed(ItemFormScreen.routeName,
                                          arguments: ItemFormScreenArguments(
                                              title: 'Edit Item',
                                              action: 'edit',
                                              item: item));
